<template>
  <div class="rounded-[12px] border border-gray-200 bg-white p-4 shadow-sm space-y-4">
    ...
    <div class="space-y-3">
      ...
      <!-- 如果没有字幕 -->
      <div v-if="!chineseSubtitle && !englishSubtitle" class="text-sm text-gray-500 text-center py-4">
        暂无字幕内容
      </div>
    </div>
    ...
    <div class="flex items-center justify-between pt-2 border-t border-gray-100">
      <div class="text-xs text-gray-500">
        {{ chineseSubtitle ? '中文字幕已准备就绪' : '等待字幕生成...' }}
      </div>
      <div class="flex items-center gap-2">
        <!-- 自动写入中文字幕按钮 -->
        <button 
          v-if="chineseSubtitle && !autoWritten"
          @click="autoWriteChinese" 
...
export default defineComponent({
  ...
  props: {
    ...
    videoTitle: String,
    autoWrite: { type: Boolean, default: true }, // 是否自动写入中文字幕
  },
  ...
  methods: {
    ...
    viewSubtitle(type, content) {
      this.modalType = type
      this.modalTitle = type === 'zh' ? '中文字幕内容' : '英文字幕内容'
    ...
    // 自动写入中文字幕
    autoWriteChinese() {
      if (this.chineseSubtitle && !this.autoWritten) {
        this.writeToInput(this.chineseSubtitle)
