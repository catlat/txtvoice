  ...
  <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
    <div class="w-full max-w-sm p-4 bg-white rounded shadow">
      ...
      <div class="mt-3 space-y-3">
        ...
        <input v-model="password" type="password" class="w-full px-3 py-2 border rounded" placeholder="输入密码" />
      </div>
      <div class="flex justify-end gap-2 mt-4">
        <button class="px-3 py-2 bg-gray-200 rounded" @click="$emit('update:modelValue', false)">取消</button>
        <button class="px-3 py-2 text-white bg-gray-900 rounded transition-colors duration-200 hover:bg-black" :disabled="!canSubmit" @click="onLogin">登录</button>
      </div>
...
import { emitAuthLogin } from '../utils/events'
    ...
    async onLogin() {
      ...
      try {
        ...
        if (data && data.token) {
          ...
          try { emitAuthLogin(data.token, this.phone) } catch (e) {}
